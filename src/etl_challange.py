...
with open("C:/Users/aashishj/Documents/jefferies/etl-challenge-v1-dsr/src/config.yaml", "r") as stream:
    try:
...
class epl_league():
    ...
    def points(self, epl_df):
        """ add columns which will keep records of point won or lost by a team"""
    ...
    def home_team(self, epl_df):
        """this method will create df for home team"""

        groupby_cols = ['HomeTeam', 'Season', 'LeagueDivision']

        epl_df_home = epl_df.groupby(groupby_cols)\
            .agg({'home_wins' : 'count', 'home_loss' : 'count', 'home_points' : 'sum', 'draw' : 'count',\
                'FullTimeResult' : 'count', 'FullTimeHomeTeamGoals' : 'sum', 'FullTimeAwayTeamGoals': 'sum'})\
                .reset_index()\
                    .rename(columns={'FullTimeResult' : 'total_home_matches', 'FullTimeHomeTeamGoals' : 'goals_for_home',\
    ...
    def away_team(self, epl_df):
        """this method will create df for away team"""

        groupby_cols = ['AwayTeam', 'Season', 'LeagueDivision']

        epl_df_away = epl_df.groupby(groupby_cols)\
            .agg({'away_wins' : 'count', 'away_loss' : 'count', 'away_points' : 'sum', 'draw' : 'count',\
                'FullTimeResult' : 'count', 'FullTimeAwayTeamGoals' : 'sum', 'FullTimeHomeTeamGoals': 'sum'})\
                .reset_index()\
                    .rename(columns={'FullTimeResult' : 'total_away_matches', 'FullTimeAwayTeamGoals' : 'goals_for_away',\
    ...
    def league_table(self, epl_df_home, epl_df_away):
        ...
        col_rename_home = {'HomeTeam' : 'club', 'total_home_matches' : 'matches_played', 'home_wins' : 'wins',\
            'home_loss' : 'loss', 'goals_for_home' : 'goals_for', 'goals_against_home' : 'goals_against',\
                'home_points' : 'points', 'Season' : 'season'}

        epl_df_home = epl_df_home.rename(columns = col_rename_home)

        col_rename_away = {'AwayTeam' : 'club', 'total_away_matches' : 'matches_played', 'away_wins' : 'wins',\
            'away_loss' : 'loss', 'goals_for_away' : 'goals_for', 'goals_against_away' : 'goals_against',\
                'away_points' : 'points', 'Season' : 'season'}
        ...
        league_table = league_table.groupby(groupby_cols)\
            .agg({'matches_played' : 'sum', 'wins' : 'sum', 'loss': 'sum', 'draw' : 'sum', 'goals_for' : 'sum',\
                'goals_against' : 'sum', 'points' : 'sum'})\
                    .reset_index()

        # adding draw points to 'points' column
        league_table['points'] = league_table['draw'].astype(int)+league_table['points'].astype(int)
        ...
        league_table = league_table.sort_values(['season', 'points', 'goal_difference_le', 'goal_difference_abs'], ascending=[False, False, True, True])\
    ...
    def best_scoring_team_by_season(self, league_table):
        ...
        champion_table = league_table.loc[league_table.groupby(['season'])["points"].idxmax()].sort_values(['season'], ascending=False)
    ...
    def write_excel(self, league_table, champion_table):
        ...
        writer_league = pd.ExcelWriter(basepath+outbound+"/EPL_League.xlsx", mode="w", engine="xlsxwriter", date_format='m/d/yyyy', datetime_format='m/d/yyyy')
        writer_champ = pd.ExcelWriter(basepath+outbound+"/Best_Scoring_Team.xlsx", mode="w", engine="xlsxwriter", date_format='m/d/yyyy', datetime_format='m/d/yyyy')

        row = 0
        sheet_league = 'league_table'
        sheet_champ = 'top_team_by_season'   

        league_table.to_excel(writer_league, sheet_name=sheet_league, index=False, startrow=row)

        champion_table.to_excel(writer_champ, sheet_name=sheet_champ, index=False, startrow=row)
    ...
    def main(self):
        """this method will be the starting point ETL process"""
        try:
...
def logs():
    ...
    logging.basicConfig(filename=filenm,
...
if __name__ == "__main__":
    ...
    filepath = basepath+inbound+"/*.json"
    file_list = glob.glob(filepath)

    #initialising logger
    logger = logs()
    logger.info("*************************EPL ETL Challange program started************************* \n")

    p = epl_league(file_list)
    p.main()

    logger.info("*************************EPL ETL Challange program completed successfully************************* \n")
